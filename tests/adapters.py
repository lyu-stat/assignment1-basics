...
from cs336_basics import train_bpe as train_bpe_module
    ...
    transformer.load_state_dict(
        {
            ...
            "FFN.weight1": weights["ffn.w1.weight"],
            "FFN.weight2": weights["ffn.w2.weight"],
            "FFN.weight3": weights["ffn.w3.weight"],
...
def run_transformer_lm(
    ...
    # Map the state dict keys from the reference implementation
    # to the keys expected by our TransformerLM implementation.
    state_dict = {
        "token_embedding.weight": weights["token_embeddings.weight"],
        "rmsnorm_final.gain": weights["ln_final.weight"],
        "output_projection.weight": weights["lm_head.weight"],
    }
    for layer_idx in range(num_layers):
        state_dict.update(
            {
                f"layers.{layer_idx}.mhsa.w_q.weight": weights[
                    f"layers.{layer_idx}.attn.q_proj.weight"
                ],
                f"layers.{layer_idx}.mhsa.w_k.weight": weights[
                    f"layers.{layer_idx}.attn.k_proj.weight"
                ],
                f"layers.{layer_idx}.mhsa.w_v.weight": weights[
                    f"layers.{layer_idx}.attn.v_proj.weight"
                ],
                f"layers.{layer_idx}.mhsa.w_o.weight": weights[
                    f"layers.{layer_idx}.attn.output_proj.weight"
                ],
                f"layers.{layer_idx}.rmsnorm1.gain": weights[
                    f"layers.{layer_idx}.ln1.weight"
                ],
                f"layers.{layer_idx}.ffn.weight1": weights[
                    f"layers.{layer_idx}.ffn.w1.weight"
                ],
                f"layers.{layer_idx}.ffn.weight2": weights[
                    f"layers.{layer_idx}.ffn.w2.weight"
                ],
                f"layers.{layer_idx}.ffn.weight3": weights[
                    f"layers.{layer_idx}.ffn.w3.weight"
                ],
                f"layers.{layer_idx}.rmsnorm2.gain": weights[
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    return train_bpe_module.train_bpe(input_path, vocab_size, special_tokens)
